...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ELONCOINParams"
    else
        PARAMS_DIR="$HOME/.eloncoin-params"
    fi
fi
...
Failed to fetch the ELONCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
ELONCOIN - fetch-params.sh

This script will fetch the ELONCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ELONCOIN zkSNARK parameters. Note that it is
