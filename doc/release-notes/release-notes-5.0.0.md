ELONCOIN Core version *v5.0.0* is now available from:  <https://github.com/eloncoin-project/eloncoin/releases>
...
ELONCOIN Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, ELONCOIN Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run ELONCOIN v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/eloncoin` folder to the `%APPDATA%\ELONCOINParams` directory.
...
#### Overview
Users will be able to protect their financial information by sending and receiving EMCs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://eloncoin.org .
...
* Top bar showing shield EMC balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield EMC balance.
...
See the release notes from [v4.2.0](https://github.com/ELONCOIN-Project/ELONCOIN/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed EMC supply information | No |
...
* `rawshieldsendmany`
  ...
  2. "amounts"            (array, required) An array of json objects representing the amounts to send.
     [{
       ...
       "amount":amount    (numeric, required) The numeric amount in EMC is the value
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in EMC
    "spends" : [
      {
        "spend" : n,                    (numeric, sapling) the index of the spend within vShieldedSpend
        "txidPrev" : "transactionid",   (string) The id for the transaction this note was created in
        "outputPrev" : n,               (numeric, sapling) the index of the output within the vShieldedOutput
        "address" : "eloncoinaddress",      (string) The ELONCOIN address involved in the transaction
        "value" : x.xxx                 (numeric) The amount in EMC
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zEMCsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield EMC)
  The `moneysupply` JSON return entry is the total EMC supply (the sum of all transparent and shield EMC)
...
| `getaccountaddress` | Returned the current ELONCOIN address associated with an internal account | Account system removal |
| `setaccount` | Associate a ELONCOIN address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given ELONCOIN address | Account system removal |
| `getaddressesbyaccount` | Returned a list of ELONCOIN addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a ELONCOIN address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a ELONCOIN address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zEMC balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zEMC mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zEMC mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zEMC amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zEMC to a EMC address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zEMC mints to a EMC address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zEMCs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzemcseed` | Set the wallet's deterministic zEMC seed to a specific value | Zerocoin deprecation |
| `getzemcseed` | Return the current deterministic zEMC seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zEMC seed | Zerocoin deprecation |
| `dzemcstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzemc` | Do a search for deterministically generated zEMC that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_eloncoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake eloncoind and unit tests: fixing missing MacOS framework. (furszy)
