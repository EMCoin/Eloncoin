ELONCOIN Core version *v4.0.0* is now available from:  <https://github.com/lightlord1233/Eloncoin/releases>
...
You can read more details about this extensive work in ([PR #954](https://github.com/lightlord1233/Eloncoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/lightlord1233/Eloncoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [ELONCOIN Wiki](https://github.com/lightlord1233/Eloncoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
...
Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/lightlord1233/Eloncoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.
...
Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zEMC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zEMC with public spends version 4 ([PR #936](https://github.com/lightlord1233/Eloncoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zEMC), version 4 spends will also be public. A full technical writeup is available on the [ELONCOIN Wiki](https://github.com/lightlord1233/Eloncoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/lightlord1233/Eloncoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/lightlord1233/Eloncoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/lightlord1233/Eloncoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/lightlord1233/Eloncoin/pull/995)).<br>
