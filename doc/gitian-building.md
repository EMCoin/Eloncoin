...
*Setup instructions for a gitian build of ELONCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ELONCOIN
...
to the ELONCOIN GitHub release page.
...
- [Building ELONCOIN Core](#building-eloncoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/eloncoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/eloncoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/eloncoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/eloncoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/eloncoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/eloncoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/eloncoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@eloncoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/eloncoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/eloncoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/eloncoin-osx-*-build.assert
```
