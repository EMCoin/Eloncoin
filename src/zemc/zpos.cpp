...
bool CLegacyZEmcStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZEmcStake from non zc-spend", __func__);
    ...
    *this = CLegacyZEmcStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zemc stake origin", __func__);

    // All good
    return true;
}

CLegacyZEmcStake::CLegacyZEmcStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZEmcStake::GetIndexFrom() const
{
...
CAmount CLegacyZEmcStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZEmcStake::GetUniqueness() const
{
...
bool CLegacyZEmcStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zEMC stake block: height %d outside range", __func__, nHeight);
