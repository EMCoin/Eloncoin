...
CEmcStake* CEmcStake::NewEmcStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CEmcStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CEmcStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CEmcStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CEmcStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CEmcStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CEmcStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CEmcStake::GetUniqueness() const
{
    //The unique identifier for a EMC stake is the outpoint
...
const CBlockIndex* CEmcStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CEmcStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CEmcStake::ContextCheck(int nHeight, uint32_t nTime)
{
