...
const char * const ELONCOIN_CONF_FILENAME = "eloncoin.conf";
const char * const ELONCOIN_PID_FILENAME = "eloncoin.pid";
const char * const ELONCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// ELONCOIN only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ELONCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ELONCOIN
// Mac: ~/Library/Application Support/ELONCOIN
// Unix: ~/.eloncoin
#ifdef WIN32
    // Windows
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ELONCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ELONCOINParams
    // Mac: ~/Library/Application Support/ELONCOINParams
    // Unix: ~/.eloncoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ELONCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "ELONCOINParams";
#else
    // Unix
    return pathRet / ".eloncoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/eloncoin/sapling-spend.params";
        sapling_output = "/usr/share/eloncoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/eloncoin/sapling-spend.params";
            sapling_output = "/usr/local/share/eloncoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", ELONCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", ELONCOIN_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override eloncoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", ELONCOIN_PID_FILENAME));
