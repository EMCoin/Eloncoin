...
#ifndef ELONCOIN_STAKEINPUT_H
#define ELONCOIN_STAKEINPUT_H
...
public:
    CEmcStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CEmcStake* NewEmcStake(const CTxIn& txin);
    ...
    bool IsZEMC() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //ELONCOIN_STAKEINPUT_H
