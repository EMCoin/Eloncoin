...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::EMC);
    ...
    if (settings.contains("nAnonymizeElonCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeeloncoinamount", settings.value("nAnonymizeElonCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eloncoin.cpp during shotdown to also remove the window positions
