...
#include "qt/eloncoin/forms/ui_addresseswidget.h"
#include "qt/eloncoin/addresslabelrow.h"
#include "qt/eloncoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(ELONCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto emcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(emcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(emcAdd);
        if (walletModel->updateAddressBookLabels(emcAdd, label.toUtf8().constData(),
