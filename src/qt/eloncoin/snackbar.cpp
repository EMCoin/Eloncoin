...
#include "qt/eloncoin/forms/ui_snackbar.h"
#include "qt/eloncoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ELONCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ELONCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
