...
#include "qt/eloncoin/dashboardwidget.h"
#include "qt/eloncoin/forms/ui_dashboardwidget.h"
#include "qt/eloncoin/sendconfirmdialog.h"
#include "qt/eloncoin/txrow.h"
...
DashboardWidget::DashboardWidget(ELONCOINGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ELONCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("EMC")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isEmc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZEMC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair EMC, zEMC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            emc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zemc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalEmc += pair.first;
            chartData->totalZemc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEmc.append(emc);
        chartData->valueszEmc.append(zemc);

        int max = std::max(emc, zemc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesEmc);
    set1->append(chartData->valueszEmc);
