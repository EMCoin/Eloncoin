...
#include "qt/eloncoin/send.h"
#include "qt/eloncoin/forms/ui_send.h"
#include "qt/eloncoin/addnewcontactdialog.h"
#include "qt/eloncoin/qtutils.h"
#include "qt/eloncoin/sendchangeaddressdialog.h"
#include "qt/eloncoin/optionbutton.h"
#include "qt/eloncoin/sendconfirmdialog.h"
#include "qt/eloncoin/guitransactionsutils.h"
#include "qt/eloncoin/loadingdialog.h"
...
SendWidget::SendWidget(ELONCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-emc");
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent EMCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield EMCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent EMCs to shield."));
    }
}
...
void SendWidget::onEMCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto emcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(emcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(emcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(emcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
