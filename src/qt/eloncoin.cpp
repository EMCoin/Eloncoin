...
#include "config/eloncoin-config.h"
#endif
...
#include "qt/eloncoin/splash.h"
#include "qt/eloncoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("eloncoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in eloncoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in eloncoin.qrc)
...
/** Class encapsulating ELONCOIN Core startup and shutdown.
...
/** Main ELONCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (ELONCOINGUI)
...
{
    window = new ELONCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ELONCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ELONCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ELONCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ELONCOINGUI::DEFAULT_WALLET);
        ...
        // ELONCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ELONCOINGUI::handlePaymentRequest);
        connect(window, &ELONCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. ELONCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(eloncoin_locale);
    Q_INIT_RESOURCE(eloncoin);
    ...
    /// 6. Determine availability of data directory and parse eloncoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("ELONCOIN Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("ELONCOIN Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // eloncoin: links repeatedly have their payment requests routed to this process:
