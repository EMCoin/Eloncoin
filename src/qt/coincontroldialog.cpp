...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select EMC Outputs to Spend" :
            "Select Shielded EMC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::EMC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show ELONCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
