...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s EMC too low! (Should be %s EMC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s EMC too high! (Should be %s EMC)" % (str(fee), str(target_fee)))
