...
class ElonCoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing eloncoind/eloncoin-cli (default: %default)")
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'eloncoind exited' in str(e)  # node must have shutdown
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as eloncoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in eloncoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in eloncoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "eloncoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run eloncoinds:
    ...
    ### ELONCOIN Specific TestFramework ###
    ###################################
...
class ComparisonTestFramework(ElonCoinTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some eloncoind binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "eloncoind"),
                          help="eloncoind binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "eloncoind"),
                          help="eloncoind binary to use for reference nodes (if any)")
...
'''
ElonCoinTestFramework extensions
'''

class ElonCoinTier2TestFramework(ElonCoinTestFramework):
