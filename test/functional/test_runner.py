...
`test/functional/test_framework/test_framework.py:ElonCoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/eloncoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and eloncoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, keep_cache="rewrite"):
    # Warn if eloncoind is already running (unix only)
    try:
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/eloncoind' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/eloncoin-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie eloncoinds, we can apply a
