...
class ReorgStakeTest(ElonCoinTestFramework):
    ...
    def check_money_supply(self, expected_emc):
        # verify that nodes have the expected EMC supply
        emc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(emc_supply, [DecimalAmt(expected_emc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that EMC supply was properly updated after the reorgs
        self.log.info("Check EMC supply...")
